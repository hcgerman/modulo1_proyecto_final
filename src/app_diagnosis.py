...
def load_data(df_entry_od: str, parent_dir: str) -> list[Diagnosis]:
    try:
        diagnosises_od = []
...
def add_diagnosises_os(
    diagnosises_loaded: list[Diagnosis], df_entry_os: pd.DataFrame, parent_dir: str
):
...
def get_diadnosis_records() -> list[Diagnosis]:
    parent_dir = os.path.join(os.path.dirname(__file__), "..")
    df = convert_xls_to_df(
        os.path.join(
            parent_dir, "data", "load_data", "ClinicalData", "diagnosis_data.xlsx"
        ),
        "od",
    )
    diagnosises = load_data(df, parent_dir)
    df_os = convert_xls_to_df(
        os.path.join(
            parent_dir, "data", "load_data", "ClinicalData", "diagnosis_data.xlsx"
        ),
        "os",
    )
    add_diagnosises_os(diagnosises, df_os, parent_dir)
